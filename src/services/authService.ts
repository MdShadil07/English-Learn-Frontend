...
class AuthService {
  ...
  async getProfile(accessToken: string): Promise<{ success: boolean; message: string; data?: any }> {
    try {
  ...
  async updateProfile(accessToken: string, data: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const response = await fetch(`${this.baseURL}/auth/profile`, {
        method: 'PUT',
        headers: {
